...
#include "RenderingAPIs/DX11/SPK_DX11LineTrailRenderer.h"
...
namespace DX11
{
	...
	SimpleMath::Vector3* DX9LineTrailRenderer::vertexBuffer = NULL;
	SimpleMath::Vector3* DX9LineTrailRenderer::vertexIterator = NULL;
	SimpleMath::Color* DX9LineTrailRenderer::colorBuffer = NULL;
	SimpleMath::Color* DX9LineTrailRenderer::colorIterator = NULL;
	...
	{
		ArrayBuffer<SimpleMath::Vector3>* fVertexBuffer = NULL;
		ArrayBuffer<SimpleMath::Color>* fColorBuffer = NULL;
		FloatBuffer* fValueBuffer = NULL;

		if ((fVertexBuffer = dynamic_cast<ArrayBuffer<SimpleMath::Vector3>*>(group.getBuffer(VERTEX_BUFFER_NAME,nbSamples))) == NULL)
			return false;

		if ((fColorBuffer = dynamic_cast<ArrayBuffer<SimpleMath::Color>*>(group.getBuffer(COLOR_BUFFER_NAME,nbSamples))) == NULL)
			return false;
	...
	{
		DX11Info::getDevice()->CreateVertexDeclaration(LineTrailVertexDecl, &pVertexDecl);

		ArrayBuffer<SimpleMath::Vector3>* fVertexBuffer = dynamic_cast<ArrayBuffer<SimpleMath::Vector3>*>(group.createBuffer(VERTEX_BUFFER_NAME,ArrayBufferCreator<SimpleMath::Vector3>(nbSamples + 2),nbSamples,true));
		ArrayBuffer<SimpleMath::Color>* fColorBuffer = dynamic_cast<ArrayBuffer<SimpleMath::Color>*>(group.createBuffer(COLOR_BUFFER_NAME,ArrayBufferCreator<SimpleMath::Color>(nbSamples + 2),nbSamples,true));
	...
	void DX9LineTrailRenderer::render(const Group& group)
	{
		...
		for(size_t i = 0; i < group.getNbParticles(); ++i)
		{
			...
			else
			{
				...
				if (count > 0)
				{
					...
					{
						std::memmove(vertexIterator + vertexDelta - 1, vertexIterator + vertexDelta, (nbSamples - count) * sizeof(SimpleMath::Vector3));
						std::memmove(colorIterator + colorDelta - 1, colorIterator + colorDelta, (nbSamples - count) * sizeof(SimpleMath::Color));
						...
						if (count > 0) // confounds points (not supposed to happen often)
							...
							{
								std::memcpy(vertexIterator + i, vertexIterator + count, sizeof(SimpleMath::Vector3));
								std::memcpy(colorIterator + i, colorIterator + count, sizeof(SimpleMath::Color));
				...
				std::memcpy(vertexIterator - 1, vertexIterator, sizeof(SimpleMath::Vector3));
				...
				*(valueIterator++) = age;
				...
				std::memcpy(vertexIterator - 1, vertexIterator - 2, sizeof(SimpleMath::Vector3));
			}
		}
		...
		std::memcpy(ptr, vertexBuffer, group.getNbParticles() * (nbSamples + 2) * sizeof(SimpleMath::Vector3));
		DX9VertexBuffer->Unlock();

		DX9ColorBuffer->Lock(0, 0, &ptr, 0);
		std::memcpy(ptr, colorBuffer, group.getNbParticles() * (nbSamples + 2) * sizeof(SimpleMath::Color));
		DX9ColorBuffer->Unlock();

		LPDIRECT3DDEVICE9 device = DX11Info::getDevice();
		device->SetVertexDeclaration(pVertexDecl);
		device->SetStreamSource(0, DX9VertexBuffer, 0, sizeof(SimpleMath::Vector3));
		device->SetStreamSource(1, DX9ColorBuffer, 0, sizeof(SimpleMath::Color));
	...
	bool DX9LineTrailRenderer::DX9CreateBuffers(const Group& group)
	{
		...
		if( DX11Info::getDevice()->CreateVertexBuffer(group.getParticles().getNbReserved() * (nbSamples + 2) * sizeof(SimpleMath::Vector3), 0, D3DFVF_XYZ, D3DPOOL_DEFAULT, &vb, NULL) != S_OK ) return false;
		std::pair<const Group *, int> key(&group, DX11_VERTEX_BUFFER_KEY);
		DX9Buffers[key] = vb;

		DX9VertexBuffer = vb;

		if( DX11Info::getDevice()->CreateVertexBuffer(group.getParticles().getNbReserved() * (nbSamples + 2) * sizeof(SimpleMath::Color), 0, D3DFVF_DIFFUSE, D3DPOOL_DEFAULT, &vb, NULL) != S_OK ) return false;
